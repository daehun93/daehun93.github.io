...
공포게임을 많이 찾아보고 해봤지만 별다른 감흥이 없던 와중

파스모포비아를 접하고 정말 재밌게 플레이 했다.
...
출시된지 2달도 안지났는데 인기게임 맨 위를 차지한 파스모포비아
...
중국인 일본인 언어의 장벽을 넘어 귀신을 잡기 위해서 

간단한 영어로 전부 소통가능하며 다들 소통 한다.
...
많으면 많을 수록 각자 역할을 나누고 플레이 시간이 단축된다.

#### 게임의 목표
![3](../assets/img/work/포모5.png)
-귀신이 출몰하는 집에 도착한 유령헌터들

게임의 목표는 어떤 귀신이 이집에 이장소에 있는지
...
게임을 시작하면 이렇게 팀원들과 본부트럭에서 시작하는데 목표 칠판이 있다.
...
원래 게임내에서 기본장비로 아주 적은 템들만 주는데

(손전등은 초등학교 문방구 손전등을 줘서 켜도 앞이 잘 보이지도 않는다.

게임을 처음 하는 사람이라면 고급 손전등을 꼭 들고다니자)
...
결론적으로 게임을 즐겨본 결과 왜 지금 스팀에서 핫 한 게임인지 알 수 있었다.

출시 된지 얼마 안된 게임이라 각종 작은 오류들이 있었는데 내가 찾아낸 
...
게임 내에서 팀원들끼리 마이크는 되는데 옵션에서 마이크 인식을 못하는 경우가 있고
...
우선 개발자들이 열심히 추가하고 있겠지만 맵의 종류가 다양하지 않다.

물론 난이도가 있어서 나중에 난이도를 높이면서 게임플레이를 하면 계속 신선도를 

유지할 수 있지만 벌써부터 고인물들이 생기고 있기 때문에 맵을 추가해야 하며

더 다양한 귀신 이벤트를 만들어 내는것도 중요하다.

지금은 싱크대가 틀어지거나 전기가 다 나가거나 액자가 떨어지고 문이 닫히는

이런 이벤트 뿐만아니라 더 다양한 이벤트가 있으면 유저들에게 공포감을 줄 수 있을꺼 같다.

출시된지 2달도 안됬지만 이렇게 많은 관심을 받고 있으니 

공포 스릴 게임으로써 데바데 처럼 유저들에게 정복 안당하려면

전문가들과 초보자들을 위한 시스템들이 지속적으로 업데이트 되어야 한다고 생각한다.
